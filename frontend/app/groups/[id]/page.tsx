...
export default function GroupDetails() {
  ...
  const handlePay = async (requestId: string) => {
    try {
      ...
      if (!createResponse.ok) throw new Error('Failed to create payment request');
      const { paymentRequest } = await createResponse.json();

      // Process the payment
      const payResponse = await fetch(`/api/payments/process`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          groupId: params.id,
      ...
      if (!payResponse.ok) throw new Error('Failed to process payment');
      await loadGroupData();
    } catch (error) {
      console.error('Error:', error);
